"""`DocumentFragment` is a "lightweight" or "minimal" [[Document]] object.
   ...
   is desirable to have an object which can hold such fragments and it is quite
   ...
   heavyweight object, depending on the underlying implementation. What is really
   needed for this is a very lightweight object. `DocumentFragment` is such an object.
   ...
   of any sub-trees defining the structure of the document. `DocumentFragment` nodes do not
   need to be [well-formed XML documents] (although they do need to follow the rules imposed
   upon well-formed XML parsed entities, which can have multiple top nodes). For example, a
   `DocumentFragment` might have only one child and that child node could be a [[Text]] node.
   ...
   may take children) the children of the `DocumentFragment` and not the `DocumentFragment`
   ...
   interface, such as [[Node.insertBefore]] and [[Node.appendChild]].

   [well-formed XML documents]: http://www.w3.org/TR/2004/REC-DOM-Level-3-Core-20040407/glossary.html#dt-well-formed
   """
